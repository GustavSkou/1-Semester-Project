...
class CommandAnswer : BaseCommand, ICommand
{
    ...
    this is done to insure that all commands works the same way. This will trough polytheism insure that all commands that are added to the "commands" dictionary, will work in the same manner. Since the "commands" dictionary contains a string as a key, and the command value is stored as an ICommand.
    ...
    {
        /* In the CommandAnswer class the Execute method is implement, so that when it is executed, it start by checking weather or not the context is currently in a question, and that the question is null. 
